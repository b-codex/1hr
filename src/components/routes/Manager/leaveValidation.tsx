...
import { validateBatchLeaveRequest, validateLeaveRequest } from '@/backend/api/LM/validateLeaveRequest';
...
const LeaveValidation = () => {
        ...
        if (hrSettings.length > 0) {
            ...
            reportees?.forEach(rID => {
                data.forEach((doc) => {
                    ...
                    if (rID === doc.employeeID && doc.leaveStage === "Requested") {
    ...
    const refuseLeave = (id: string) => {
        Modal.confirm({
            ...
            cancelText: 'No',
            onOk: async () => {
                await refuseLeaveRequest(id, "LM")
                    .then((res: boolean) => {
                        if (res) {
                            message.success('Success.');
                        }
    ...
    const validateBatchLeave = () => {
        Modal.confirm({
            ...
            cancelText: 'No',
            onOk: async () => {
                await validateBatchLeaveRequest(selections)
                    .then((res: boolean) => {
                        message.success("Success");
                    })
                    .catch((err: any) => {
    ...
    const refuseBatchLeave = () => {
        Modal.confirm({
            ...
            cancelText: 'No',
            onOk: async () => {
                await refuseBatchLeaveRequest(selections, "LM")
                    .then((res: boolean) => {
                        message.success("Success");
                    })
                    .catch((err: any) => {
    ...
    return (
        <>
            <DashboardCard title="Leave Validation" action={<SideButtons />}>
